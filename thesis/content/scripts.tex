...
\begin{algorithm}
	...
	\KwIn{parsed arguments $\xi$}
	 ...
	 \If{parsed load parameter $\iota \in \xi$}{
	 	Load policy model $\pi$
	 }
	 \Else{
	 	Create policy model $\pi$
	 }
...
\begin{algorithm}
	...
	\label{alg:curri}
	\KwIn{total steps $t_{total}$,  set of environment args $\lambda$, model $\pi$, number of parallel environments $\eta$, name}
	
	Get radius $R_{max}$ from $\lambda$
	
	$R = 0.0$
	
	\While{$R \leq R_{max}$}{
		Change radius in $\lambda$ ro R
...
\begin{algorithm}
	...
	\For{$j=0 \to e$}{
		\Repeat{$done$}{
			Get action $a$ with the use of $\pi$
			
			Step $\epsilon$ and get the new observation $\sigma$, reward $r$, $done$ and $info$
