...
\begin{algorithm}
	...
	\While{$R \leq R_{max}$}{
		...
		$steps \leftarrow \min (\lfloor t_{total} \cdot \frac{\delta}{\delta + 1} \rfloor, rest)$
		
		Learn the model $\pi$ with the amount of $steps$ 
		
		Save $\pi$ and rename the best policy $\pi_{best}$
		
		$R \leftarrow R + \delta \cdot R_{max}$

		$rest \leftarrow rest - steps$

		\If{$R > R_{max}  \land rest$}{
			$R \leftarrow R_{max}$
		}
	}
\end{algorithm}
