...
def run(duration_sec: int, physics: Physics, simulation_freq_hz: int, control_freq_hz: int,
    ...
    # Init environment #################################################################################################
    env = CtrlAviary(drone_model=drone, num_drones=1, initial_xyzs=INIT_XYZS, initial_rpys=INIT_RPYS,
                     physics=physics, neighbourhood_radius=10, freq=simulation_freq_hz,
                     aggregate_phy_steps=AGGR_PHY_STEPS, gui=True, record=False, obstacles=False, user_debug_gui=False)
    debug(bcolors.OKGREEN, 'Environment created successfully')
    ...
    for i in range(0, int(duration_sec * env.SIM_FREQ), AGGR_PHY_STEPS):
        ...
        # Make it rain #################################################################################################
        if 5 < i / env.SIM_FREQ < 10 and i % 10 == 0:
            p.loadURDF("duck_vhacd.urdf", [0 + random.gauss(0, 0.3),
        ...
        p.applyExternalForce(env.DRONE_IDS[0],
                             ...
                             forceObj=[random.gauss(wind_x, 0.001),
                                       random.gauss(wind_y, 0.001),
                                       random.gauss(wind_z, 0.001)],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--physics', default=Physics("pyb"),
                        type=Physics, help='The chosen Physics (standard PYB)', metavar='')
    ...
    parser.add_argument('--target_x', default=0, type=float,
                        help='X Coordinate of Target', metavar='')
    parser.add_argument('--target_y', default=0, type=float,
                        help='Y Coordinate of Target', metavar='')
    parser.add_argument('--target_z', default=0.5, type=float,
                        help='Z Coordinate of Target', metavar='')
    parser.add_argument('--wind_x', default=0, type=float,
                        help='Wind force in x direction', metavar='')
    parser.add_argument('--wind_y', default=0, type=float,
                        help='Wind force in y direction', metavar='')
    parser.add_argument('--wind_z', default=-0, type=float,
                        help='Wind force in z direction', metavar='')
